...
{ 
	u8 res;
	// MPU6050_IIC_Init();//初始化MPU6050_IIC总线
...
{ 
	...
	while(len)
	{
		if(len==1)*buf = MyI2C_ReceiveByte(0);//读数据,发送nACK 
		else *buf = MyI2C_ReceiveByte(1);		//读数据,发送ACK  
		len--;
		buf++; 
	}    
    MyI2C_Stop();	//产生一个停止条件 
...
u8 MPU_Read_Byte(u8 reg)
{
    ...
    MyI2C_Wait_Ack();		//等待应答 
	res=MyI2C_ReceiveByte(0);//读取数据,发送nACK 
    MyI2C_Stop();			//产生一个停止条件 
	return res;		
}
